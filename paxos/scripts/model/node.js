...
define(["./log_entry"], function (LogEntry) {
    function Node(model, id) {
        ...
        this.addEventListener("coordinatorIdChange", this.onCoordinatorIdChange);
        this.addEventListener("votedForChange", this.onVotedForChange);
        this.addEventListener("voteCountChange", this.onVoteCountChange);
        this.addEventListener("currentSeqIdChange", this.onCurrentSeqIdChange);
    ...
    /**
     * Initializes the node to a replica.
     */
    ...
    /**
     * Retrieve who the node voted for in this seqid.
     */
    ...
    /**
     * Retrieve the number of votes received for this seqid.
     */
    ...
    /**
     * Retrieve the current known coordinator identifier.
     */
    Node.prototype.coordinatorId = function () {
    ...
    Node.prototype.matchIndex = function (id, value) {
        ...
        // Only update if this node is the coordinator.
    ...
    /**
     * Sets or retrieves the current SeqId
     */
    Node.prototype.currentSeqId = function (value) {
        if (arguments.length === 0) {
            return this._currentSeqId;
        }
        var changed = (value > this._currentSeqId),
            prevValue = this._currentSeqId;
        if (changed) {
            this._currentSeqId = value;
            this._votedFor = null;
            this._coordinatorId = null;
            this._voteCount = 0;
            this.state("replica");
            ...
            this.dispatchChangeEvent("currentSeqIdChange", value, prevValue);
        }
        return this._value;
    };
    ...
    Node.prototype.state = function (value) {
        ...
        switch (this._state) {
        case "coordinator":
            this.coordinatorEventLoop();
            break;
        case "proposer":
            this.proposerEventLoop();
            break;
        case "replica":
            this.replicaEventLoop();
            break;
        case "stopped":
    ...
    Node.prototype.execute = function (command, callback) {
        ...
        // Append to log.
        this._log.push(new LogEntry(this.model(), prevIndex + 1, this.currentSeqId(), command, callback));
        this.dispatchChangeEvent("logChange");
    };
    ...
    /**
     * Runs the node as a coordinator:
     *
     ...
     *
     *   - If last log index ≥ nextIndex for a replica: send AE
     *     RPC with log entries starting at nextIndex.
     *     - If succcessful: update nextIndex and matchIndex for replica (§5.3).
     *     - If AE fails because of log inconsistency: decrement nextIndex
     *       and retry (§5.3).
     *
     *   - If there exists an N such that N > commitIndex, a majority of
     *     matchIndex[i] ≥ N, and log[N].seqid == currentSeqId:
     *     set commitIndex = N (§5.3, §5.4).
     */
    Node.prototype.coordinatorEventLoop = function () {
        var self = this,
            frame = this.frame();
     ...
     *
     *   - On conversion to proposer, start election:
     *     - Increment currentSeqId.
     *     - Vote for self.
     *     - Reset election timeout.
     *     - Send RequestVote RPCs to all other servers.
     *
     *   - If votes received from majority of server: become coordinator.
     *
     *   - If AppendEntries RPC received from new coordinator: convert to replica.
     *
     *   - If election timeout elapses, start new election.
     */
    Node.prototype.proposerEventLoop = function () {
        // Increment current seqid.
        ...
        this._voteCount = 1;
        this._coordinatorId = null;
        this.dispatchChangeEvent("coordinatorIdChange");
        this.dispatchChangeEvent("votedForChange");
        this.dispatchChangeEvent("voteCountChange");
    ...
    /**
     * Runs the node as a replica.
     *
     *   - Respond to RPCs from proposers and coordinators.
     *
     *   - If election timeout elapses without receiving AE RPC
     *     from current coordinator or granting vote to proposer:
     *     convert to proposer.
     */
    Node.prototype.replicaEventLoop = function () {
        this.clearHeartbeatTimer();
        this.resetElectionTimer();
    };
    ...
    Node.prototype.sendRequestVoteRequest = function (target) {
        var self  = this,
            ...
            req   = {
                ...
                lastLogSeqId: (prevEntry !== undefined ? prevEntry.seqid : 0),
            };
     ...
     *
     *   - Reply false if seqid < currentSeqId (§5.1).
     *
     *   - If votedFor is null or proposerId, and proposer's log is at
    ...
    Node.prototype.recvRequestVoteRequest = function (source, req) {
        var self  = this,
            ...
            prevLogSeqId  = (prevLogEntry !== undefined ? prevLogEntry.seqid : 0),
            voteGranted = true;
        ...
        this.currentSeqId(req.seqid);

        // Reply false if seqid < currentSeqId (§5.1).
        if (req.seqid < this.currentSeqId()) {
            voteGranted = false;
        }
        // Reply false if already voted for another proposer.
        else if (this._votedFor !== null && this._votedFor !== req.proposerId) {
            voteGranted = false;
        }
        // Reply false if proposer's log is not at least as up-to-date as receiver's log.
        else if (req.lastLogSeqId < prevLogSeqId || (req.lastLogSeqId === prevLogSeqId && req.lastLogIndex < prevLogIndex)) {
            voteGranted = false;
        }

        if (voteGranted) {
            // Vote for proposer.
            this._votedFor = req.proposerId;
            this.dispatchChangeEvent("votedForChange");
    ...
    Node.prototype.recvRequestVoteResponse = function (source, req, resp) {
        ...
        this.currentSeqId(resp.seqid);

        if (resp.voteGranted && this.state() === "proposer") {
            this._voteCount += 1;
    ...
    Node.prototype.recvAppendEntriesRequest = function (source, req) {
        ...
        // Reply false if seqid < currentSeqId (§5.3).
        if (req.seqid < this.currentSeqId()) {
            success = false;
        }
        // Reply false if log doesn't contain an entry at prevLogIndex whose
        // seqid matchs prevLogSeqId (§5.3).
        else if (req.prevLogIndex !== 0 && (prevEntry === undefined || prevEntry.seqid !== req.prevLogSeqId)) {
            success = false;
        }

        if (success) {
            // Update coordinator.
            this._coordinatorId = req.coordinatorId;
            this.dispatchChangeEvent("coordinatorIdChange");

            // Step down if proposer.
            if (this.state() === "proposer") {
                this.state("replica");
            }

            // If an existing entry conflicts with a new one (same index but different seqids), delete the existing entry and all that follow it (§5.3).
            ...
            // If coordinatorCommit > commitIndex, set commitIndex = min(coordinatorCommit, last log index).
            this.commitIndex(req.coordinatorCommit);
    ...
    Node.prototype.onCoordinatorIdChange = function (event) {
        event.target.layout().invalidate();
    };
    ...
    Node.prototype.onCurrentSeqIdChange = function (event) {
        event.target.layout().invalidate();
    };
