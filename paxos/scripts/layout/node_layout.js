        ...
        this.g().selectAll(".node").data(nodes, function (d) { return d.id; })
            .call(function () {
                var transform = function(d) { return "translate(" + self.parent().scales.x(d.x) + "," + self.parent().scales.y(d.y) + ")"; },
                    ...
                    stroke = {
                        dash: function (d) { return (d.state() === "proposer" ? "5,5" : ""); },
                        opacity: function (d) { return (d.state() === "replica" || d.state() === "stopped" ? 0 : 1); },
                    };
                ...
                g.each(function(node) {
                    ...
                    desc.push("SeqId: " + node.currentSeqId());
                    desc.push();
                    if (node.state() === "replica") { 
                        desc.push("Coordinator: " + node.coordinatorId());
                    } 
            ...
            .call(function () {
                this.select("path.election-timeout")
                    .attr("d", function(d) {
                        ...
                        return d3.svg.arc()
                            .innerRadius(self.parent().scales.r(r - 1 - (d.state() === "proposer" ? 2 : 0)))
                            .outerRadius(self.parent().scales.r(r - (d.state() === "proposer" ? 2 : 0)) + 1)
                            .startAngle(0)
